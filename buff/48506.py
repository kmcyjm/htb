...
#   Remote Attackers to gain Remote Code Execution (RCE) on the Hosting Webserver via uploading a maliciously
#   crafted PHP file that bypasses the image upload filters.
# Exploit Details:
...
#   4. Bypass the file type check by modifying the 'Content-Type' of the 'file' parameter to 'image/png'
#   in the POST request, and set the 'pupload' paramter to 'upload'.
...
#   6. The Web Application will rename the file to have the extension with the second item in an array created from
#   the file name; seperated by the '.' character.
